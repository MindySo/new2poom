...
class LazyQwenTryOnPipeline:
    ...
    def tryon_clothes_v2(self, person_image_path, extracted_clothes_path, output_path):
        """Stage 2: Try on with IMPROVED face preservation using structured prompts"""
        print(f"Trying on clothes (V2 - Face Protection)...")
        ...
        # STRUCTURED PROMPT for face preservation (color patch based)
        structured_prompt = (
            "Remove the black color patch. "
            "A portrait of the person in image 1 "
            "wearing the outfit in image 2. "
            "Keep the face, head, and facial features exactly the same as image 1. "
            "Only change the clothing and body area below the neck."
        )

        # NEGATIVE PROMPT to prevent face modification
        negative_prompt = (
            "blurry face, distorted face, changed facial features, "
            "different person, face modification, ugly face, deformed face, "
            "western face, caucasian face, multiple faces"
        )

        print(f"  Using structured prompt for face preservation...")

        # Try on clothing with improved parameters
        result = self.pipe(
...
def create_face_with_body_template_v2(face_image_path, output_path):
    """
    V2: Crop face and add body template with BLACK MASK on face
    Black mask signals to Qwen: "Don't modify this area!"
    """
    print(f"Creating face + body template with protection mask...")
    print(f"  From: {os.path.basename(face_image_path)}")
    ...
    if len(faces) > 0:
        ...
        # === KEY IMPROVEMENT: Add BLACK MASK over face area ===
        # This tells Qwen to preserve this area!
        mask_expand = 0.1  # Slightly smaller than face to ensure coverage
        face_mask_x1 = int(face_w * mask_expand)
        face_mask_y1 = int(face_h * mask_expand)
        face_mask_x2 = int(face_w * (1 - mask_expand))
        face_mask_y2 = int(face_h * (1 - mask_expand))

        # Draw BLACK rectangle over face (protection signal)
        cv2.rectangle(
            canvas,
            (face_mask_x1, face_mask_y1),
            (face_mask_x2, face_mask_y2),
            (0, 0, 0),  # BLACK color
            thickness=-1  # Filled
        )

        print(f"  → Black protection mask added over face area")

        # Draw body silhouette (light gray)
        ...
        print(f"  → Face + body template with mask: {result_pil.size}")
...
def process_missing_person_case_tryon_v2(case_id):
    """Process case with Qwen Try-On V2 (Face Protection)"""
    print(f"\n{'='*60}")
    print(f"Processing: {case_id} (QWEN TRY-ON V2 - FACE PROTECTION)")
    print(f"{'='*60}\n")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Step 1: Create face + body template WITH BLACK MASK
        face_body_template_path = os.path.join(temp_dir, "face_body_template_v2.jpg")
        face_body_template, face_detected = create_face_with_body_template_v2(
            face_image,
        ...
        # Step 3: Try on clothing with V2 (improved face preservation)
        final_output = os.path.join(temp_dir, "final_result.jpg")
        result_image = lazy_qwen_tryon.tryon_clothes_v2(
            face_body_template_path,  # Face with BLACK mask
        ...
        analysis_result = {
            ...
            "processing_method": "Qwen_Try_On_V2_Face_Protection_Mask",
...
def process_all_cases_tryon_v2():
    """Process all cases with Qwen Try-On V2"""
    s3_handler = S3Handler()
    ...
    for case_id in cases:
        try:
            process_missing_person_case_tryon_v2(case_id)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print("Processing all cases (QWEN TRY-ON V2)...")
        process_all_cases_tryon_v2()
    elif len(sys.argv) == 2:
        case_id = sys.argv[1]
        print(f"Processing: {case_id} (QWEN TRY-ON V2)")
        process_missing_person_case_tryon_v2(case_id)
    else:
        print("Usage:")
        print("  python main_qwen_tryon_v2_s3.py                 # All cases")
        print("  python main_qwen_tryon_v2_s3.py <case_id>       # Specific case")
        sys.exit(1)
