...
def load_police_css():
    ...
    * {
    ...
    /* Video container for smooth streaming */
    .video-container {
        border-radius: 16px;
        overflow: hidden;
            ...
            if start_btn:
                try:
                    ...
                    if not cap.isOpened():
                        ...
                        # 최적화 변수
                        display_update_interval = 1  # 디스플레이는 매 1프레임마다 (원래는 2였지만 1로 유지)
                        metric_update_interval = 10   # 메트릭은 10프레임마다
                        display_scale = 0.8           # 디스플레이 해상도 80%
                        jpeg_quality = 75             # JPEG 품질 75%
                        ...
                        while st.session_state.get('webcam_running', False):
                            ...
                            # 🚀 최적화 1: 디스플레이 업데이트 간격 조절
                            if processed_count % display_update_interval == 0:
                                # 🚀 최적화 2: 디스플레이용 해상도 축소
                                frame_display = cv2.resize(frame, (display_width, display_height))

                                # 🚀 최적화 3: JPEG 압축으로 전송량 감소
                                encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_quality]
                                _, buffer = cv2.imencode('.jpg', frame_display, encode_param)

                                # 🚀 최적화 4: Base64 + HTML로 직접 렌더링 (더 빠름)
                                img_base64 = base64.b64encode(buffer).decode()
                                frame_placeholder.markdown(
                                    f'<div class="video-container"><img src="data:image/jpeg;base64,{img_base64}" style="width:100%; display:block;"></div>',
                                    unsafe_allow_html=True
                                )

                            # 🚀 최적화 5: 메트릭 업데이트 빈도 감소
                            if processed_count % metric_update_interval == 0:
                                col_m1.metric("처리 프레임", f"{processed_count:,}")
                                col_m2.metric("탐지 횟수", f"{detection_count}")
